...
describe('Ng2SmartTableComponent', () => {
    ...
    it('should emit rowSelect when selecting a row', async () => {
        await renderTable(baseSettings(), new LocalDataSource([
            { id: 1, name: 'a' },
            { id: 2, name: 'b' },
        ]));
        const rowSelectSpy = vi.fn().mockName('rowSelect');
        const rowDeselectSpy = vi.fn().mockName('rowDeselect');
        ...
        expect(vi.mocked(rowSelectSpy).mock.lastCall[0].data.id).toBe(2);
        expect(vi.mocked(rowSelectSpy).mock.lastCall[0].isSelected).toBe(true);
    });

    it('should emit rowDeselect when clicking an already selected row', async () => {
        await renderTable({ ...baseSettings(), selectedRowIndex: -1 }, new LocalDataSource([{ id: 1, name: 'a' }]));
        const rowSelectSpy = vi.fn().mockName('rowSelect');
        const rowDeselectSpy = vi.fn().mockName('rowDeselect');
        component.rowSelect.subscribe(rowSelectSpy);
        component.rowDeselect.subscribe(rowDeselectSpy);
        ...
        expect(vi.mocked(rowDeselectSpy).mock.lastCall[0].isSelected).toBe(false);
        expect(rowSelectSpy).toHaveBeenCalledTimes(1);
    });
    ...
    it('should emit rowDeselect for the default auto-selected first row', async () => {
        ...
        expect(rowDeselectSpy).toHaveBeenCalledTimes(1);
        expect(vi.mocked(rowDeselectSpy).mock.lastCall[0].data.id).toBe(1);
        expect(vi.mocked(rowDeselectSpy).mock.lastCall[0].isSelected).toBe(false);
    ...
    it('should filter rows when source filter changes', async () => {
        ...
        // Apply filter then refresh the view.
    ...
    it('should emit rowHover when hovering a row', async () => {
        await renderTable(baseSettings(), new LocalDataSource([{ id: 1, name: 'a' }]));
        const hoverSpy = vi.fn().mockName('rowHover');
